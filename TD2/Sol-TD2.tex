...
\subsubsection*{Énoncé}
Comme nous l'avons vu en cours. L'utilisation des plus petits points préfixes permet de définir de façon uniforme la clôture d'une relation sous une certaine propriété (transitivité, réflexivité, symétrie, etc.). Cependant, il est possible dans certains cas de définir ces clôtures d'une façon plus maniable. Chacune des équations ci-dessous propose une telle définition; démontrer-les.
...
Montrer que $\mu F^2$ est un plus petit point préfixe  de $F$\\ 

Montrons que $\mu F^2$ est un point préfixe.\\
\begin{align*}
    &F(\mu F^2) \subseteq \mu F^2 \\
    \iff &F(\mu F^2) \subseteq F^2(\mu F^2)\\ 
    \Leftarrow & \mu F^2 \subseteq F(\mu F^2) \\ 
    \Leftarrow & F^3(\mu F^2) \subseteq F(\mu F^2)\\
    \iff F^3(\mu F^2) \subseteq F^3(\mu F^2) && \square
\end{align*}

Montrons que c'est le plus petit.\\ 

Soit $R$ tq $F(R) \subseteq R$
\begin{align*}
    &\mu F^2 \subseteq R\\ 
    \Leftarrow & F^2(R) \subseteq R\\ 
    \Leftarrow & F^2(R) \subseteq F(R)\\ 
    \Leftarrow & F(R) \subseteq R && \square
\end{align*}
